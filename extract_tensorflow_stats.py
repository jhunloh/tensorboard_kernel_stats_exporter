...
def write_stats(data, format, path):
    """Writes kernel_stats to a file and adds file type extension."""
    file_name = os.path.join(path, tool + '.' + format)
    with open(file_name, "w") as f:
        f.write(data)


def export_stats(data_files):
    """Exports the stats data for each file in `data_files` and saves them as csv and json next to the corresponding *.xplane.pb"""
    for data in data_files:
        ...
        data_csv, _ = raw_to_tool_data.xspace_to_tool_data([data], tool + '^', {'tqx' : 'out:csv'})
        data_json, _ = raw_to_tool_data.xspace_to_tool_data([data], tool + '^', {'tqx' : 'out:json'})
        
        path, _ = os.path.split(data)
        write_stats(data_csv, 'csv', path)
        write_stats(data_json, 'json', path)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
                    prog=os.path.basename(__file__),
                    description='Extracts framework_op_stats from *.xplane.pb files in <logpath>',
                    epilog='')
